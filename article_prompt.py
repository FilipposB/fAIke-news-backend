...
def is_version_valid(article):
    ...
    return article_version == VERSION or article_version in SUPPORTED_VERSIONS


def extract_article(api_key, api_key_gog, cse_id, topic, topicPretty, word_limit=600, variation=0.3, mock=False):
    new_word_limit = random.random() * variation * word_limit

    secure_prompt = SecurePromptLibrary(api_key=api_key,  word_limit=new_word_limit)
    google_image_api = GoogleImageApi(api_key_gog, cse_id)

    rules = [
        "Weave in **0 to 5 quotes** from 'interviewees' — real or imagined — but make them feel natural, spontaneous, and occasionally a little too perfect, like references overheard in a documentary no one can find.",
        "Write in **structured, flowing paragraphs** — no bullet-point nonsense or half-baked thoughts allowed.",
        "Make it **unputdownably fascinating**. If it doesn’t make someone lean in, laugh, or squint suspiciously, rewrite it.",
        "Walk the line: it must sound **plausible**… while being **utterly ridiculous**. Think: ‘I *could* believe that… maybe?’",
        "Keep it **anchored in reality** — Earth only — but stretch that reality until it squeaks. Aliens in Ohio? Maybe. Interdimensional squirrels? Nope.",
        "Maintain a tone of **dry irony or detached amusement**, like the writer knows something you don’t — but won’t admit it.",
        "Credit the work to a **clearly fake author name** — something like 'Mallory Vex' or 'Chester Blunt'. No real people.",
        "Wrap the final result in **valid JSON** with these fields: `headline`, `article_body`, `keywords`, `author`, and `google_image_query`. The JSON must start with `{` and end with `}`. All keys must be relevant and help users find the story or visualize it.",
        "The `headline` should read like a click-worthy news headline — short, punchy, and slightly suspicious.",
        "The `google_image_query` should be a search that would return a compelling, weirdly appropriate image to match the article (ex: 'man holding giant carrot in rain')."
    ]
    ...
    if mock:
        content = f'''json
                    ...
                    "headlines": {topicPretty},
                    "topic": {topic}
                    }
                '''
    else:
        content = secure_prompt.generate_content(topic=topicPretty, rules=rules)
